var lucsonpay = require('./index.js');
const http = require('http');
const { parse } = require('querystring');

const hostname = '127.0.0.1';
const port = 3000;
const lucsonpay_payid = 'XXXXXXXXXXXXXXXX';
const lucsonpay_salt = 'XXXXXXXXXXXXXXXX';
const server = http.createServer((req, res) => {
    lucsonpay.isProdMode(false);
    lucsonpay.setCredentails(lucsonpay_payid, lucsonpay_salt);
    switch (req.url) {
        case "/":
        res.statusCode = 200;
        ...
        res.write('<title>Lucsonpay Demo Merchant Checkout Page</title>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="MERCHANTNAME" class="signuptextfield" value="Lucsonpay Demo" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>');
        res.write('<tr>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="CUST_NAME" class="signuptextfield" value="LUCSONPAY DEMO" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>                   ');
        res.write('<tr>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="CUST_EMAIL" class="signuptextfield" value="test.user@lucsonpay.com" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>');
        res.write('<tr>');
            ...
            collectRequestData(req, result => {
                if (result) {
                    var data = {
                        ...
                        PAY_ID: lucsonpay_payid,
                        PRODUCT_DESC: result.PRODUCT_DESC,
                        RETURN_URL: 'http://' + hostname + ':' + port + '/response',
                        TXNTYPE: 'SALE'
                    }
                    var transaction = lucsonpay.createTransaction(data);
                    var gateway_url = lucsonpay.getPaymentUrl();
